...
verifyBlocksPrefix blocks eos =
    case splitEpochs blocks of
      ...
      ESRValid genEpoch (OldestFirst succEpochs) -> do
        ...
        let pm = ccMagic
        verify $ validateGenEpoch pm eos ccHash0 ccInitLeaders genEpoch >>= \genUndos -> do
          epochUndos <- sequence $ validateSuccEpoch pm eos <$> succEpochs
  ...
  where
    ...
    validateGenEpoch pm eos ccHash0 ccInitLeaders geb = do
    ...
    validateSuccEpoch :: ProtocolMagic
                      ...
                      -> EpochBlocks NE
                      -> Verify VerifyBlocksException (OldestFirst NE Undo)
    validateSuccEpoch pm eos (SuccEpochBlocks ebb emb) = do
      Verify.verifyBlocksPrefix
        pm
        eos
        (ebb ^. headerHashG)
