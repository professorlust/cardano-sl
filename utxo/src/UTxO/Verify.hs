...
verifyBlocksPrefix
    ...
    -> EpochOrSlot   -- ^ Current EpochOrSlot
...
verifyBlocksPrefix pm eos tip curSlot leaders lastSlots blocks = do
    ...
    slogUndos <- mapVerifyErrors VerifyBlocksError $
                   slogVerifyBlocks curSlot eos leaders lastSlots blocks
    ...
    txUndo <- mapVerifyErrors (VerifyBlocksError . pretty) $
        tgsVerifyBlocks pm eos $ map toTxpBlock blocks
...
slogVerifyBlocks curSlot eos leaders lastSlots blocks = do
    ...
    verResToMonadError formatAllErrors $
        verifyBlocks dummyConfig eos curSlot dataMustBeKnown adoptedBVD leaders blocksList
...
tgsVerifyBlocks pm eos newChain = do
    bvd <- gsAdoptedBVData
    ...
    let verifyPure :: [TxAux] -> Verify VerifyBlockFailure TxpUndo
        verifyPure txs = nat $
          withExceptT (verifyBlockFailure txs) $
            verifyToil pm eos bvd mempty epoch dataMustBeKnown txs
