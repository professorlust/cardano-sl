...
checkTxGood :: Tx -> EpochOrSlot -> Bool
checkTxGood tx eos = isRight $ checkTx eos tx

checkTxBad :: Tx -> EpochOrSlot -> Bool
checkTxBad UnsafeTx {..} eos =
    all (\outs -> isLeft $ checkTx eos (UnsafeTx _txInputs outs _txAttributes)) badOutputs
  where
...
txGen size = do
    ...
    eos <- arbitrary
    case checkTx eos tx of
        Left e   -> error $ "txGen: something went wrong: " <> e
        Right () -> pure tx

-- Values that test when to enforce the
-- Attributes size Tx validation rule

beforeCutoffEpochIndex :: EpochOrSlot
beforeCutoffEpochIndex =
    epochOrSlotFromSlotId
        $ SlotId (EpochIndex 99) (UnsafeLocalSlotIndex 0)

beforeCutoffLocalSlotIndex :: EpochOrSlot
beforeCutoffLocalSlotIndex =
    epochOrSlotFromSlotId
        $ SlotId (EpochIndex 100) (UnsafeLocalSlotIndex 4)

afterCutoffEpochIndex :: EpochOrSlot
afterCutoffEpochIndex =
    epochOrSlotFromSlotId
        $ SlotId (EpochIndex 101) (UnsafeLocalSlotIndex 0)

afterCutoffLocalSlotIndex :: EpochOrSlot
afterCutoffLocalSlotIndex =
    epochOrSlotFromSlotId
